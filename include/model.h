...
public:
	...
	TextureMesh TextureFromFile(const char *path, const std::string &directory, bool gamma)
	{
		TextureMesh texture;
...
private:
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		// 4. height maps
		std::vector<TextureMesh> heightMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_height");
	...
	std::vector<TextureMesh> loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
	{
		...
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				TextureMesh texture = TextureFromFile(str.C_Str(), this->directory, false);
